...
export interface LiveSessionRepository {
  startLiveSession(lat: number, lng: number, accuracy: number | null): Promise<LiveSessionResult | null>;
  updateLiveSession(shareId: string, lat: number, lng: number, accuracy: number | null): Promise<void>;
  endLiveSession(shareId: string): Promise<void>;
}
